...
```{r}
scfa_indv <- read_excel("~/Documents/UMich_Bio201_F19/Lab3/raw_data/SCFA_data.xlsx",
...
nrow() # 4078 Number of rows 
ncol() # 19 Number of columns
dim() # 4078   19  Number of rows and columns 
colnames() # This tells you the names of all the columns 
rownames() # This tells you the row numbers up until 1000. 3078 entries are omitted 
glimpse() # This tels you the values in each column along with their data types. There are 4,078 observations and 19 variables. Shows as much data as possible.
str() # Displays the internal structure of the R object. Tells you the values in each column along with their data types. It displays abbreviated contents of lists.
```
...
f18_but_rps <- scfa_indv %>%
  ...
  filter(sample_wt_g >= 0.10 & sample_wt_g <= 1) %>%
  filter(notes != "frozen >24hrs") %>%
  select(participant_id, sex, age, ht_in, wt_lbs, sample_number, sample_id, study_week, semester, supplement_consumed, sample_wt_g, butyrate_mM)
```


# Calculations across column
...
scfa_indv_qc <- scfa_indv %>%
  ...
  rename(race_ethnicity = "race/ethnicity",
         sample_wt_g = tube_wt) %>%
  mutate(wt_kgs = wt_lbs / 2.205) %>%
  mutate(ht_m = ht_in / 35.274) %>%
  filter(sample_wt_g >= 0.10 & sample_wt_g <= 1) %>%
  filter(notes != "frozen >24hrs") %>%
  mutate(sample_wt_kgs = sample_wt_g / 1000) %>%
  mutate(acetate_mmol_kg = ace / sample_wt_kgs) %>%
  mutate(butyrate_mmol_kg = but / sample_wt_kgs) %>%
  mutate(propionate_mmol_kg = pro / sample_wt_kgs) %>%
  mutate(total_scfa = acetate_mmol_kg + butyrate_mmol_kg + propionate_mmol_kg) %>%
mutate(total_acetate_rounded = round(acetate_mmol_kg, 2)) %>%
mutate(total_butyrate_rounded = round(butyrate_mmol_kg, 2)) %>%
mutate(total_propionate_rounded = round(propionate_mmol_kg, 2)) %>%
mutate(total_scfa_rounded = round(total_scfa, 2)) 
...
```{r}
write_delim(scfa_indv_qc, path = "~/Documents/UMich_Bio201_F19/Lab3/curated_data/scfa_indv_qc.txt", delim = "\t")
```
...
```{r}
scfa_wkly_qc <- read_excel(path = "/Users/knayak/Documents/UMich_Bio201_F19/Lab3/raw_data/SCFA_data.xlsx", 
                           sheet = "SCFA_wkly", 
                           col_names = TRUE, 
                           trim_ws = TRUE, 
                           na = c("", "NA", "-----")) %>%
  ...
  mutate(total_scfa_mean = mean(total_scfa))  %>%
  mutate(total_scfa_median = median(total_scfa))
  
```

Export this curated data frame:
```{r}
write_delim(scfa_wkly_qc, path = "curated_data/SCFA_data.xlsx", delim = "\t")
```
