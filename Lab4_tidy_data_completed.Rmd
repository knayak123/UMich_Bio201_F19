...
```{r}
scfa_data <- read_delim(file = "~/Documents/UMich_Bio201_F19/Lab3/curated_data/scfa_indv_qc.txt",
...
scfa_data %>%
  ...
  group_by(semester, supplement_consumed) %>%
  mutate(mean_butyrate = mean(butyrate_mmol_kg)) %>%
  mutate(mean_propionate = mean(propionate_mmol_kg)) %>%
  mutate(mean_acetate = mean(acetate_mmol_kg)) 
```
...
scfq_wkly <- scfa_data %>%
  ...
  filter(!is.na(acetate_mmol_kg)) %>%
  filter(!is.na(propionate_mmol_kg)) %>%
  group_by(semester, study_week, supplement_consumed) %>%
  mutate(mean_butyrate = mean(butyrate_mmol_kg)) %>%
  mutate(mean_propionate = mean(propionate_mmol_kg)) %>%
  mutate(mean_acetate = mean(acetate_mmol_kg)) 
...
```{r}
scfa_wkly <- scfa_data %>%
  select(participant_id, semester, supplement_consumed, study_week, butyrate_mmol_kg, acetate_mmol_kg, propionate_mmol_kg) %>% 
...
```{r}
# write code here
butyrate_difference <- scfa_wkly %>%
  select(participant_id, semester, supplement_consumed, study_week, mean_butyrate) %>% 
  filter(supplement_consumed == "BRMPS" | supplement_consumed == "Accessible" | supplement_consumed == "HiMaize") %>% 
  filter(study_week == "week1" | study_week == "week3") %>% 
  spread(supplement_consumed, mean_butyrate) %>% 
  na.omit(BRMPS, Accessible, HiMaize) %>% 
  mutate(delta_butyrate_difference = week3 - week1) 
...
```{r}
# copy+paste and edit code chunk for acetate
acetate_difference <- scfa_wkly %>%
  select(participant_id, semester, supplement_consumed, study_week, mean_acetate) %>% 
  filter(supplement_consumed == "BRMPS" | supplement_consumed == "Accessible" | supplement_consumed == "HiMaize") %>% 
  filter(study_week == "week1" | study_week == "week3") %>% 
  spread(supplement_consumed, mean_acetate) %>% 
  na.omit(BRMPS, Accessible, HiMaize) %>% 
  mutate(delta_acetate_difference = week3 - week1) 
```

```{r}
# copy+paste and edit code chunk for propionate
propionate_difference <- scfa_wkly %>%
  select(participant_id, semester, supplement_consumed, study_week, mean_propionate) %>% 
  filter(supplement_consumed == "BRMPS" | supplement_consumed == "Accessible" | supplement_consumed == "HiMaize") %>% 
  filter(study_week == "week1" | study_week == "week3") %>% 
  spread(supplement_consumed, mean_propionate) %>% 
  na.omit(BRMPS, Accessible, HiMaize) %>% 
  mutate(delta_propionate_difference = week3 - week1) 
```

```{r}
# copy+paste and edit code chunks for plots
plots_difference <- scfa_wkly %>%
  select(participant_id, semester, supplement_consumed, study_week, mean_plots) %>% 
  filter(supplement_consumed == "BRMPS" | supplement_consumed == "Accessible" | supplement_consumed == "HiMaize") %>% 
  filter(study_week == "week1" | study_week == "week3") %>% 
  spread(supplement_consumed, mean_plots) %>% 
  na.omit(BRMPS, Accessible, HiMaize) %>% 
  mutate(delta_plots = delta_propionate_difference + delta_acetate_difference + delta_butyrate_difference) 
```
...
full_join(propionate_wide, 
            by = c("participant_id", "semester", "supplement_consumed")) %>%
mutate(wkly_total_scfa = acetate_mmol_kg + butyrate_mmol_kg + propionate_mmol_kg) %>%
mutate(delta_weekly_total_scfa = week3 - week1)
nrow(join1)
n_distinct(join1$participant_id)
colnames(join1)
```
...
```{r}
plot_semester_change <- scfa_data %>%
  filter(study_week == "week1" | study_week == "week3") %>%
  ggplot(aes(x = semester, 
             y = total_scfa, 
             BRMPS, Arabino, HiMaize, Inulin, Accessible, BRMPS)) +
  geom_point(aes(color = semester)) +
  facet_grid(~semester) + 
  labs(x = "Semester",
       y = "Total SCFA (mmol/kg)") +
  theme(axis.text.x = element_blank(),
        legend.position = "none")

plot_semester_change
```
...
```{r}
scfa_long <- total_scfas
join2 <- inner_join(ace_long, but_long,
                    by = c("participant_id", "semester", "supplement_consumed")) %>%
full_join(pro_long, 
            by = c("participant_id", "semester", "supplement_consumed")) %>%
nrow(join2)
...
wkly_means <- scfa_long %>%
   gather(key = scfa , value = concentration_mmol_kg,
         mean_acetate, mean_butyrate, mean_propionate)
```
...
```{r}
write_delim(scfa_wide, path = "~/Documents/UMich_Bio201_F19/Lab4/curated_data/scfa_wide.txt", delim = "\t")
```
...
```{r}
# import pH data
pH_data <- read_delim(file = "~/Documents/UMich_Bio201_F19/Lab4/raw_data/pH_indv.txt",
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA")) %>%
  rename_all(tolower)
  filter(participant_id, study_week, ph, use_data == "yes")

# filter to remove bad measurements based on outside list
pH_bad <- read_delim(file = "~/Documents/UMich_Bio201_F19/Lab4/raw_data/pH_bad.txt",
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA")) %>% #import list of samples with date discrepencies 
  rename_all(tolower)
  filter(sample_id, date_collected, participant_id, ph, flag == "", comments = "")
  
pH_qc <- pH_data %>%
  filter(!sample_id %in% pH_bad$sample_id) %>% #drop samples in bad list
  group_by(participant_id, sample_id, date_collected, ph) %>%
  
  summarise(sample_id, ph) #calculate weekly average
dim(pH_qc)
```

```{r}
# import BSS
BSS_data <- read_delim(file = "~/Documents/UMich_Bio201_F19/Lab4/raw_data/BSS_indv.txt",
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, 
                        na = c("", "NA")) %>%
  rename_all(tolower)
...
BSS_qc <- BSS_data %>%
  ...
  group_by(study_week, bristol_score, bristol_numeric) %>%
  summarise(bristol_numeric, bristol_score) #calculate weekly average 
dim(BSS_qc)
```
...
pH_BSS_wkly <- inner_join(pH_qc, BSS_qc,
                    by = c()) %>%
  mutate(round(avg = bristol_numeric + pH)) #round avgs to reflect sig figs
```
...
```{r}
write_delim(pH_BSS_wkly, path = "~/Documents/UMich_Bio201_F19/Lab4/curated_data/pH_BSS_wkly.txt", 
