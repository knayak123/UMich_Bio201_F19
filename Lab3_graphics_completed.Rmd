...
scfa_qc <- read_delim(file = "~/Documents/UMich_Bio201_F19/curated_data/SCFA_wkly.txt", 
                        delim = "\t", escape_double = FALSE, trim_ws = TRUE, na=c("NA"),
                        col_types = list())
  rename_all(tolower) %>%
  filter(quantity_compliant == "yes") %>%
  select(-acetate_median, -butyrate_median, -propionate_median) %>%
  rename(acetate_mean_mmol_kg = "acetate_mean") %>%
  rename(butyrate_mean_mmol_kg = "butyrate_mean") %>%
  rename(propionate_mean_mmol_kg = "propionate_mean") %>%
  mutate(scfa_total_mmol_kg = acetate_mean_mmol_kg + butyrate_mean_mmol_kg + propionate_mean_mmol_kg) 
scfa_qc
...
```{r}
plot_acetate <- ggplot(data = scfa_qc,
                   mapping = aes(x = participant_id,
                                 y = acetate_mean_mmol_kg,
                                 color = study_week)) + 
  geom_point() +
   labs(title = "Mean Acetate Concentrations",
         x = "Individuals",
...
```{r}
plot_butyrate <- ggplot(data = scfa_qc,
                   mapping = aes(x = participant_id,
                                 y = butyrate_mean_mmol_kg,
                                 color = study_week)) + 
   geom_point() +
   labs(title = "Mean Butyrate Concentrations",
         x = "Individuals",
         y = "Fecal butyrate (mmol/kg)") +
  theme(axis.text.x = element_blank())
plot_butyrate
...
```{r}
plot_propionate <- ggplot(data = scfa_qc,
                   mapping = aes(x = participant_id,
                                 y = propionate_mean_mmol_kg,
                                 color = study_week)) + 
   geom_point() +
   labs(title = "Mean Propionate Concentrations",
         x = "Individuals",
...
```{r}
plot_total <- ggplot(data = scfa_qc,
                   mapping = aes(x = participant_id,
                                 y = scfa_total_mmol_kg,
                                 color = study_week)) + 
   geom_point() +
   labs(title = "Mean Total Concentrations",
         x = "Individuals",
         y = "Total SCFA Presence (mmol/kg)") +
  theme(axis.text.x = element_blank())
plot_total
```
...
> ** Graphs lacking facets were not divided very well. This was seen where some of the weeks looked like separate graphs rather than parts of the same graph. Geom_blank also caused the generated graph to be expanded a bit. The generated graph above was more varied with its options of colors, understandable, and made proper use of space due to accurate point representation. ** 
...
plot3 <- scfa_qc %>%
    filter(study_week == "week3") %>%
    filter(supplement_consumed == "accessible" | supplement_consumed == "arabino" | supplement_consumed ==     
           "brmps" | supplement_consumed == "brmps+accessible" | supplement_consumed == "himaize" | 
            supplement_consumed == "himaize+brmps" | supplement_consumed == "inulin" | supplement_consumed == 
            "loodat" | supplement_consumed == "psyllium" | supplement_consumed == "psyllium+brmps" | 
            supplement_consumed == "transition_himaize") %>%
  ggplot(aes(x = participant_id,
             y = butyrate_mean_mmol_kg, semester, study_week)) +
  geom_point(aes(color = supplemment_consumed)) +
...
```{r}
# option to save with ggplot functions 
ggplot2::ggsave(filename = "~/Documents/UMich_Bio201_F19/Lab3_graphics.rmd",
                path = "figures/",
                device = "tiff",
                plot = plot3_6,
                dpi = "retina")
