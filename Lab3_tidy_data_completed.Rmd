...
scfa_indv_qc <- scfa_indv %>%
  ...
  mutate(acetate_mmol_kg = acetate_mm / sample_wt_kgs) %>%
  mutate(butyrate_mmol_kg = butyrate_mm / sample_wt_kgs) %>%
  mutate(propionate_mmol_kg = propionate_mm / sample_wt_kgs) %>%
...
```{r}
scfa_wkly_qc <- read_excel(path = "raw_data/SCFA_data.xlsx", 
                           sheet = "SCFA_indv", 
                           col_names = TRUE, 
                           trim_ws = TRUE, 
                           na = c("", "NA", "-----")) %>%
...
```{r}
write_delim(scfa_wkly_qc, path = "raw_data/SCFA_data.xlsx", delim = "\t")
```
