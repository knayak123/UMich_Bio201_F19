...
# save richness data 
write_delim(richness_df, path = "~/Documents/UMich_Bio201_F19/Lab7/curated_data/richness_df.txt", 
...
#variances 
var.test(wk1$avg_richness, wk3$avg_richness)

# join data frames
joined_df <- inner_join(x = wk1, y = wk3, 
...
t.test(x = joined_df$avg_richness_wk1, y = joined_df$avg_richness_wk3, 
       alternative = "less", paired = TRUE)
# p-value = 1
"Since the p-value is 1 (greater than 0.05), we fail to reject the null hypothesis. There was no significant change in average richness between weeks 1 and 3 "
```
...
# save Simpson data 
write_delim(simpson_df, path = "~/Documents/UMich_Bio201_F19/Lab7/curated_data/simpson_df.txt", 
            delim = "\t", col_names = TRUE)
```
...
shapiro.test(wk1s$avg_richness) 
ggplot(wk1s, aes(x = avg_richness)) + geom_histogram()

wk3s <- simpson_df %>%
  filter(study_week == "week3", 
         supplement_consumed == "BRMPS") 

shapiro.test(wk3s$avg_richness) 
ggplot(wk3s, aes(x = avg_richness)) + geom_histogram()

#variances 
var.test(wk1s$avg_richness, wk3s$avg_richness)

# join data frames
joined_df2s <- inner_join(x = wk1s, y = wk3s, 
    by = c("participant_id", "semester", "supplement_consumed")) %>%
  rename(avg_richness_wk1s = avg_richness.x,
         avg_richness_wk3s = avg_richness.y) %>%
  select(-starts_with("study_week"))

# write code for paired t-test here 
wilcox.test(x = joined_df2s$avg_richness_wk1s, 
            y = joined_df2s$avg_richness_wk3s, 
            alternative = "less", paired = TRUE) 

# p-value = 0.986
"Since the p-value is 0.986 (greater than 0.05), we fail to reject the null hypothesis. There was no significant change in average richness between weeks 1 and 3 "
```
